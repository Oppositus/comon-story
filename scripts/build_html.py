...
/* Таблицы: их в серии много, и они разной ширины. Узкая — не шире колонки
   текста (как абзац или картинка); широкой не хватает колонки — она
   вырывается из неё, но не больше 90% ширины страницы, а текст в ячейках
   переносится, так что горизонтальный скролл не нужен. width: max-content
   (а не fit-content(90vw) — эта форма функции не поддерживается браузерами
   для width вне grid) даёт истинную ширину по содержимому без оглядки на
   узкий родительский контейнер, а min/max-width обрезают её до коридора.
   Центрирование через transform, а не фиксированный margin в vw, работает
   при любой итоговой ширине. */
.tablewrap {
  position: relative;
  left: 50%;
...
@media (max-width: 640px) {
  ...
  .tablewrap { min-width: min(44rem, 94vw); max-width: 94vw; }
...
def convert(md_text):
    ...
    # неразрывный пробел перед знаком процента — иначе браузер переносит "%"
    # на новую строку, оставляя число висеть в конце предыдущей
    body = re.sub(r'(\d) %', r'\1&nbsp;%', body)

    # неразрывные пробелы внутри числа с разрядами по три цифры (5 134,
    # 19 978, 2 877 000) — иначе перенос рвёт число между группами
    body = re.sub(r'\d{1,3}(?: \d{3})+', lambda m: m.group(0).replace(" ", "&nbsp;"), body)
